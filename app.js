...
let state={project:{company:'ESTUDIO MISOGI',name:'Tala Villa',client:'Новый клиент',address:'Tala, Paphos',estimateNo:'EST-2026-015',defaultMargin:25},sections:[],workLog:[],library:structuredClone(initialLibrary)};
...
function syncConfig(){try{return JSON.parse(localStorage.getItem(GOOGLE_SYNC_KEY))||{url:'',token:''}}catch(e){return{url:'',token:''}}}
function pendingSync(){try{return JSON.parse(localStorage.getItem(GOOGLE_QUEUE_KEY))||[]}catch(e){return[]}}
function queueForSync(action,payload){const queue=pendingSync();queue.push({action,...payload,queuedAt:new Date().toISOString()});localStorage.setItem(GOOGLE_QUEUE_KEY,JSON.stringify(queue));flushMaterialQueue()}
function queueMaterialForSync(material){queueForSync('upsertMaterial',{material})}
function materialPayload(m){return{id:String(m.googleId||m.id),name:m.name,category:m.category,packaging:m.packaging||'',packQuantity:m.packQuantity||1,unit:m.unit,priceExVat:m.priceExVat||0,vat:m.vat??VAT,consumption:m.consumption||0,consumptionUnit:m.consumptionUnit||'',supplier:m.supplier||supplierName(m.supplierId)||'',note:m.note||''}}
async function loadMaterialsFromGoogle(){const c=syncConfig();if(!c.url)return;try{const sep=c.url.includes('?')?'&':'?';const r=await fetch(`${c.url}${sep}action=materials&token=${encodeURIComponent(c.token)}`);if(!r.ok)throw new Error('Google response '+r.status);const data=await r.json();if(!Array.isArray(data.materials))throw new Error('No materials');data.materials.forEach(raw=>{const id='GS_'+raw.id;let supplier=state.library.suppliers.find(s=>s.name===raw.supplier);if(!supplier&&raw.supplier){supplier={id:'GS_S_'+uid(),name:raw.supplier};state.library.suppliers.push(supplier)}const m={id,googleId:raw.id,category:raw.category||'Прочие материалы',manufacturer:'',name:raw.name,unit:raw.unit||'шт',buyInc:Number(raw.unitPriceIncVat||raw.priceIncVat||0),supplierId:supplier?.id||'',effectiveDate:new Date().toISOString().slice(0,10),active:true,packaging:raw.packaging,packQuantity:raw.packQuantity,priceExVat:raw.priceExVat,vat:raw.vat,consumption:raw.consumption,consumptionUnit:raw.consumptionUnit,note:raw.note};const existing=state.library.materials.find(x=>x.id===id);if(existing)Object.assign(existing,m);else state.library.materials.push(m)});persist();render();toast('Материалы загружены из Google');}catch(e){console.warn('Google materials sync failed',e);toast('Нет связи с Google — работаем офлайн')}}
async function loadWorkLogFromGoogle(){const c=syncConfig();if(!c.url)return;try{const sep=c.url.includes('?')?'&':'?';const r=await fetch(`${c.url}${sep}action=workLog&token=${encodeURIComponent(c.token)}`);if(!r.ok)throw new Error('Google response '+r.status);const data=await r.json();if(Array.isArray(data.entries)){state.workLog=data.entries;persist();renderWorkLog()}}catch(e){console.warn('Google work log sync failed',e)}}
async function flushMaterialQueue(){if(!navigator.onLine)return;const c=syncConfig();const queue=pendingSync();if(!c.url||!queue.length)return;const rest=[];for(const item of queue){try{const r=await fetch(c.url,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({...item,token:c.token})});if(!r.ok)throw new Error('Google response '+r.status)}catch(e){rest.push(item)}}localStorage.setItem(GOOGLE_QUEUE_KEY,JSON.stringify(rest));if(!rest.length)toast('Правки материалов отправлены в Google')}
function openSyncSettings(){const c=syncConfig();$('#syncUrl').value=c.url||'';$('#syncToken').value=c.token||'';$('#syncStatus').textContent=c.url?'Подключение сохранено в этом браузере.':'Подключение ещё не настроено.';openModal('syncSettingsModal')}
...
function renderWorkLog(){const body=$('#workLogBody');if(!body)return;body.innerHTML='';const entries=[...(state.workLog||[])].sort((a,b)=>String(b.date).localeCompare(String(a.date)));$('#workLogEmpty').classList.toggle('hidden',entries.length>0);entries.forEach(x=>{const row=document.createElement('tr');row.innerHTML=`<td>${x.date||''}</td><td>${x.object||''}</td><td>${x.worker||''}</td><td>${x.work||''}</td><td>${Number(x.hours||0)}</td><td>${x.status||''}</td><td>${x.note||''}</td>`;body.appendChild(row)})}
function openWorkLog(){renderWorkLog();$('#logDate').value=new Date().toISOString().slice(0,10);$('#logObject').value=state.project.name||'';openModal('workLogModal')}
...
function addLibraryMaterial(){const name=prompt('Название материала');if(!name)return;const material={id:uid(),category:'Прочие материалы',manufacturer:'',name,unit:'шт',buyInc:0,supplierId:state.library.suppliers[0].id,effectiveDate:new Date().toISOString().slice(0,10),active:true};state.library.materials.push(material);persist();queueMaterialForSync(materialPayload(material));renderMaterialsLibrary($('#materialsLibrarySearch').value);toast('Материал добавлен в справочник')}
...
$('#syncSettingsForm').onsubmit=e=>{e.preventDefault();const url=$('#syncUrl').value.trim(),token=$('#syncToken').value.trim();localStorage.setItem(GOOGLE_SYNC_KEY,JSON.stringify({url,token}));closeModals();loadMaterialsFromGoogle();loadWorkLogFromGoogle();flushMaterialQueue();toast('Синхронизация сохранена')};
$('#workLogForm').onsubmit=e=>{e.preventDefault();const entry={id:'LOG-'+uid(),date:$('#logDate').value,object:$('#logObject').value.trim(),worker:$('#logWorker').value.trim(),work:$('#logWork').value.trim(),hours:+$('#logHours').value||0,status:$('#logStatus').value,note:$('#logNote').value.trim()};if(!entry.date||!entry.object||!entry.worker||!entry.work)return;state.workLog.push(entry);persist();renderWorkLog();queueForSync('upsertWorkLog',{entry});e.target.reset();$('#logDate').value=new Date().toISOString().slice(0,10);$('#logHours').value=8;toast('Запись добавлена в журнал')};
...
$('#workLogLink').onclick=e=>{e.preventDefault();openWorkLog()};
$('#settingsLink').onclick=e=>{e.preventDefault();openSyncSettings()};
window.addEventListener('online',flushMaterialQueue);
restore();render();loadMaterialsFromGoogle();loadWorkLogFromGoogle();
