...
function queueMaterialForSync(material){const queue=pendingSync();queue.push({action:'upsertMaterial',material,queuedAt:new Date().toISOString()});localStorage.setItem(GOOGLE_QUEUE_KEY,JSON.stringify(queue));flushMaterialQueue()}
...
function openSyncSettings(){const c=syncConfig();const url=prompt('Вставьте URL Google-синхронизации',c.url||'');if(url===null)return;const token=prompt('Код доступа синхронизации',c.token||'');if(token===null)return;localStorage.setItem(GOOGLE_SYNC_KEY,JSON.stringify({url:url.trim(),token:token.trim()}));loadMaterialsFromGoogle();flushMaterialQueue()}
...
restore();render();loadMaterialsFromGoogle();
